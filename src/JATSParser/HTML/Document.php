...
class Document extends \DOMDocument
{
    ...
    public function setReferences(
        string $citationStyle = JATSPARSER_CITEPROC_STYLE_DEFAULT,
        string $lang = JATSPARSER_CITEPROC_LANG_DEFAULT,
        bool $styleInTextLinks = false
    ): void {
    ...
    public function getHtmlForTCPDF()
    {
        ...
        foreach ($tableAndFigureLinks as $tableAndFigureLink) {
            $tableAndFigureLink->setAttribute(
                "style",
                "background-color:#c6ecc6; color:#495A11; text-decoration:none;"
            );
        }
            ...
            switch (get_class($articleSection)) {
                ...
                case "JATSParser\Body\Section":
                    if ($articleSection->getTitle()) {
                        $sectionElement = $this->createElement(
                            "h" . ($articleSection->getType() + 1),
                            $articleSection->getTitle()
                        );
    ...
    protected function extractReferences(array $references): void
    {
        ...
        foreach ($references as $reference) {
            ...
            if (!$citeProcRef->refIsEmpty()) {
                ...
                $citeProcRef->getJatsReference()->isMixed()
                && !empty(trim($citeProcRef->getJatsReference()->getRawReference()))
            ) {
